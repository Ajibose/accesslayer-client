...
- `calculatePortfolioValue()` - Main aggregation function
- `formatPortfolioValueDisplay()` - Display formatting
- `getPortfolioValueHelperText()` - Helper message logic
- `calculatePositionTotalValue()` - Single position calculation
- `sortHoldingsByTotalValue()` - Sorting logic

### Data Structures

```typescript
interface HeldKeyPosition {
	creatorId: string;
...
- 1 XLM = 10,000,000 stroops (Stellar native asset precision)
- All calculations use stroops internally
- Display formatting converts to XLM with proper decimal places
...
1. **Data Integrity:** Never shows incomplete totals if any price is loading/missing/stale
2. **Reactivity:** Responds immediately to cache updates without manual refresh
3. **UX Clarity:** Clear messaging for loading, unavailable, and error states
4. **Precision:** Uses stroops internally to avoid floating-point errors
5. **Edge Cases:** Handles all null/undefined/invalid quantity scenarios

### Test Quality Metrics

- **Coverage:** 38 distinct test cases
- **Assertions:** 100+ individual assertions
- **Edge Cases:** Comprehensive (nulls, negatives, decimals, large numbers)
- **Mocking:** No external API calls required
...
These tests validate the utility functions that power the **Portfolio Total Value** feature in the LandingPage component:
...
The tests ensure that this calculation accurately reflects user holdings and responds correctly to React Query cache changes.
...
- ⏳ Integration tests in LandingPage component
- ⏳ E2E tests for complete user workflows
- ⏳ Performance testing for large portfolios (1000+ positions)
